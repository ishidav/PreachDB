% $Id: n5_peterson_modified.erl,v 1.1 2009/07/23 16:18:26 depaulfm Exp $
%------------------------------------------------------------------------------
%  LICENSE AGREEMENT
% 
...
%  PackageName                [preach]
% 
%  Synopsis                   [This is a semi-automatic translation of         ]%                             [ 7_peterson_modified.m                          ]
% 
...
transition(State) ->
  T = [ rule0(State,1), rule0(State,2),rule0(State,3), rule0(State,4),rule0(State,5),
	rule1(State,1), rule1(State,2),rule1(State,3), rule1(State,4),rule1(State,5),
	rule2(State,1), rule2(State,2),rule2(State,3), rule2(State,4),rule2(State,5),
	rule3(State,1), rule3(State,2),rule3(State,3), rule3(State,4),rule3(State,5),
	rule4(State,1), rule4(State,2),rule4(State,3), rule4(State,4),rule4(State,5)
       ],
...
rule1(State = #state{}, Field) ->
	...
	if ((((((not(Field /= 1) or (element(1,(State#state.mu_q)#mu_q.q) < element(Field,(State#state.mu_localj)#mu_localj.localj))) and 
 		  ...
 		  (not((Field /= 5)) or (element(5,(State#state.mu_q)#mu_q.q) < element(Field,(State#state.mu_localj)#mu_localj.localj)))) or
		...
		if ( Y =< 5 ) ->%%%%%%%%%%%%%%%%%%%% THIS RHS WAS HARD-CODED
			X = mu_L1;
		   true ->
			X = mu_L4
		end,
...
%======================StartState=====================
startstates() ->
   [#state{mu_q      = #mu_q{q={1,1,1,1,1}},
	  mu_p      = #mu_p{p={mu_L0,mu_L0,mu_L0,mu_L0,mu_L0}},
	  mu_turn   = #mu_turn{turn={1,1,1,1,1,1}},
	  mu_localj = #mu_localj{localj={1,1,1,1,1}}}].
