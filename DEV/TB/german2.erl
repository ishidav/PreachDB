...
%-export([start/3,startWorker/1]).
 -export([transition/1,startstate/1]).
...
% models a for loop in murphi, can probably be used
% as a basis for doing quantification too
forloop(X,[H|T],B) -> B(H,(forloop(X,T,B)));
forloop(X,[],B) -> X.

-record(cache_t,{ state, data }).
-record(msg_t,{ cmd, data }).
...
startstate(D) ->
   MS = #murphi_state{},
   MS = forloop(MS,{1,2,3},
           fun(I,MS = #murphi_state{}) ->
             MS = MS#murphi_state{
                     chan1 = setelement(I,MS#murphi_state.chan1,(MS#murphi_state.chan1)#msg_t{cmd = empty})
                  },
             MS = MS#murphi_state{
                     chan2 = setelement(I,MS#murphi_state.chan2,(MS#murphi_state.chan2)#msg_t{cmd = empty})
                  },
             MS = MS#murphi_state{
                     chan3 = setelement(I,MS#murphi_state.chan3,(MS#murphi_state.chan3)#msg_t{cmd = empty})
                  },
             MS = MS#murphi_state{
                     cache = setelement(I,MS#murphi_state.cache,(MS#murphi_state.cache)#cache_t{state = invalid})
                  },
             MS = MS#murphi_state{
   ...
   MS = MS#murphi_state{exGntd = false},
   MS = MS#murphi_state{curCmd = empty},
   MS = MS#murphi_state{memData = D},
   MS = MS#murphi_state{auxData = D},
   MS.
